...
(* Engines to run at once, at most.
   ...
   same test with one engine stalled not at all and honoured its
   timeout to a tenth of a second.
...
(* The order engines are given up in when the machine cannot carry them
   ...
   keep from the invariants another engine generates, so in a prefix too
   short to reach invariant generation it is worth less than a proof
   engine that stands on its own; it goes just after the first generator
   instead. What a machine that can carry only three should run is BMC,
   k-induction and IC3QE.

   Engines not named here keep the relative order [--enable] gave them,
   at the end. *)
let cap_priority = function
  | `BMC -> 0
  | `IND -> 1
  | `IC3 | `IC3QE -> 2
  | `BMCSKIP -> 3
...
let cap_engines_to_machine modules =
  let cap = max_engines () in
  if List.length modules <= cap then modules
  else (
    let ranked =
      List.stable_sort
        (fun a b -> compare (cap_priority a) (cap_priority b)) modules
    in
    let kept, dropped = list_split cap ranked in
    KEvent.log L_note
    ...
    (* Back to the order they were given in, so that nothing downstream
       sees the ranking *)
...
let analyze msg_setup save_results ignore_props stop_if_falsified slice_to_prop modules in_sys param sys =
  ...
  ( if TSys.has_real_property sys |> not && not ignore_props then
      ...
      let modules = cap_engines_to_machine modules in
