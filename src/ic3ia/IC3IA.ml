...
let sys_def solver sys offset =
  let init =
    let init_flag_term =
      TransSys.init_flag_of_bound sys offset
    in
    let init_flag_var =
      Var.mk_state_var_instance
        (TransSys.init_flag_state_var sys)
        offset
    in
    Term.mk_and
      [TransSys.init_of_bound
        ...
        sys
        offset
      |> Term.apply_subst [(init_flag_var, Term.mk_not init_flag_term)];
        (TransSys.init_flag_of_bound sys (Numeral.pred offset))
      ]
   in
   let trans =
    Term.mk_and
...
let check_sat solver term =
  ...
  if SMTSolver.check_sat solver then(
    ...
    SMTSolver.pop solver ; Some model
  ) else (
    SMTSolver.pop solver ; None
  )

let extract_cube sys predicates model =
  let eval p =
    match Eval.eval_term (TransSys.uf_defs sys) model p with
    | Eval.ValBool true -> p
...
let get_bad_cube solver sys predicates frames prop =
  let r_and_not_p =
    Term.mk_and [get_R frames (depth frames); Term.mk_not prop]
  in
  match check_sat solver r_and_not_p with
  | Some model -> (
    Some (extract_cube sys predicates model)
  )
  | None -> None
...
let is_blocked solver frames s =
  ...
  if blocked then
    ...
    in
    match check_sat solver query with
    | None -> true
    | _ -> false
  )
...
let get_init sys =
  TransSys.init_flag_of_bound sys base_offset

let solve_relative solver sys predicates frames s option =
  ...
  if (SMTSolver.check_sat solver) then (
    let c =
      ...
      | ExtractModel ->
         let model = SMTSolver.get_model solver in
         extract_cube sys predicates model
      | _ -> Cube.empty
    in
...
let generalize solver sys init predicates frames s =
    ...
    List.fold_left
      (fun c l ->
        ...
        if (is_initial init c') then
          ...
          let z =
            solve_relative solver sys predicates frames s' Default
          in
...
let get_invariant init c =
  List.fold_left
    (fun disj l ->
      if not (l == (Term.mk_not init)) (* && l <> A *) then (* TODO *)
        Term.negate l :: disj
      else
        disj
    )
      ...
      | _ -> (
        let next_cube, frames, q =
          if not (is_blocked solver frames s) then (
            ...
            let z =
              solve_relative solver sys predicates frames s ExtractModel
            in
            ...
            | _ -> (
              ...
              let z =
                generalize solver sys init predicates frames z
              in
              ...
              let rec push_forward z' =
                ...
                | Frame f when f < (depth frames) - 1 -> (
                  let nz =
                    ...
                    solve_relative solver sys predicates frames nxt Default
                  in
        ...
        CS.fold
          (fun c frames' ->
            ...
            let s =
              ...
              solve_relative solver sys predicates frames' tc NoInd
            in
...
let main in_sys param sys =
   ...
   (* let sys_props =
     TransSys.props_list_of_bound sys base_offset
   in *)

   let real_props =
     TransSys.props_list_of_bound sys base_offset
     |> List.filter (fun (n,_) -> TransSys.is_candidate sys n |> not)
   in

   let prop_name, prop_term =
     match real_props with
     | [(n,t)] -> n, t
     | _ -> failwith ("IC3IA: Only a single property supported for now!")
   in

   let init = get_init sys in

   let prop = Term.mk_or [prop_term; init] in

   (* let predicates =
     List.fold_left
       (fun acc (_,t) -> TS.add t acc)
       (Term.get_atoms init)
       sys_props
   in *)
   ...
   try (
    ...
    let rec loop predicates next_cube frames =
      match get_bad_cube solver sys predicates frames prop with
      | Some cube -> (
        let s =
