...
(** Block until an event is queued for the calling domain, and at most
    for a fixed period. Returns immediately if there are queued events.
    Termination requests arrive as messages too, so a waiting engine is
    woken up on shutdown and raises {!Terminate} in its next call to
    {!recv}.

    The wait is bounded so that an engine waiting for something that
    never arrives as a message keeps rechecking its own state instead
    of blocking forever. A return means that something may have
    happened, not that a message arrived. *)
