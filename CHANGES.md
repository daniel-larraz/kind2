...
Improvements:
- The exit code of a compositional analysis now reflects its final verdict. A property falsified under the abstraction of a callee and then proved once the callee was refined left the run exiting with the code for a falsified property (40), or for an incomplete analysis (30) when it had been left open rather than falsified: the safety judgement scanned every analysis of a node rather than the latest. The latest analysis of a node now decides; an earlier falsification only counts when the latest analysis is itself incomplete.
- Fixed a runtime failure at the end of a refinement analysis (`could not find system ... in abstraction map of previous result`). The summary of the analysis lists the systems it refined by looking each concrete system up in the abstraction map of the previous analysis, and a system that is only reached through a node the previous analysis abstracted by its contract, such as an unrolling of a recursive function called from its body, has no entry there. Such a system is new rather than refined, and is now skipped.
- Fixed a crash of the integer and real invariant generators on a system with algebraic datatypes (`Caught exception in invariant generator: Invalid_argument("num_of_value for term ...")`): a field selector applied to a value built with another constructor has no value in a model, and one applied to a matching value came back as an opaque atom rather than a numeral. Candidate terms with a selector are now left out, and the simplifier evaluates a selector on a matching value to the normal form of the field.
...
- A call to a recursive function that Kind 2 defines at the SMT level (one with no contract to abstract its calls with, or declared `transparent`) may now be applied to a quantified variable. The call is compiled to an application of the symbol the function is defined as rather than to an instance of the function, so the quantifier is free to range over its arguments, and a property such as `forall (n: Nat) Even(Succ(n)) = not Even(n)` is now accepted. The contract checker and assumption generation decline a model with such a call, since they build their system without functional constraints and rest on quantifier elimination, which does not take an uninterpreted function. A call to a recursive function that a contract abstracts stays rejected: its symbol is uninterpreted away from the arguments of its instances, so it would stand for an arbitrary function under the quantifier. So does a call whose argument holds a symbolic array index rather than a variable of an explicit quantifier. When the definition is left out for a reason only known once the model is compiled (the body is not a total function of its inputs, or `--define_fun_rec false`), the call is accepted and Kind 2 warns that the function is an arbitrary function of its inputs under the quantifier.
...
Breaking changes:
- The contract of a `transparent` recursive function is no longer assumed in place of its body at the recursion cutoff, since the function now has an SMT-level definition that says exactly what it is and that the contract would contradict if a guarantee did not hold of it. Its guarantees stay proof obligations but no longer act as the induction hypothesis of the recursion, so one that needs induction over the recursion is no longer proved; state it in a lemma, or drop the `transparent` modifier.
