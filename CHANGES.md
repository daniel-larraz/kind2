...
- Support for [recursive functions](https://kind.cs.uiowa.edu/docs/main/user/inputs-and-outputs/lustre/#recursive-functions), declared with the `rec` modifier, including mutual recursion. Termination is established through a mandatory `decreases` clause over integer or ADT measures. A recursive function is defined at the SMT level with `define-funs-rec` (when using Z3 or cvc5), so that the solver can unfold its definition. In a compositional analysis, its recursive calls are abstracted by its contract instead, and when the analysis is also modular a call to it from another node is refined to the definition once the function's own analysis has proved its contract. A function with no contract, or declared `transparent`, is always defined; an `opaque` one is always abstracted by its contract.
- Support for the [subset and superset](https://kind.cs.uiowa.edu/docs/main/user/inputs-and-outputs/sets/) operators `<=`, `<`, `>=`, and `>` on sets.

Improvements:
- A modular analysis (`--modular true`) analyzes every node after the nodes it calls. It used to analyze a node before one of its callees when the caller listed that callee after another node that also calls it, so the callee's contract was not yet known to hold when the caller was analyzed, and a compositional analysis of the caller could not refine the call.
- The exit code of a compositional and modular analysis reflects the last refinement of each node, as the final summary does. A property that was falsified under the abstraction of a subnode and then proved after refining that subnode was still reported as falsified by the exit code.
...
- Kind 2's machine-readable output is now parseable in cases where it was not. Several of them made the whole document unusable rather than the one field at fault, and the regression suite reads exit codes alone, so none of them were noticed there; it now checks that both formats parse.
  - Strings written into JSON output are escaped as JSON requires. A property expression that pretty-prints over several lines carried its newlines raw into the `expr` field; backslashes (every separator of a Windows path, for one) were passed through unescaped, and tabs and other control characters were not escaped either. Double quotes, which used to be replaced by single quotes, are now escaped and so survive a round trip.
  - Strings written into XML output are escaped as XML requires. A property name holding a double quote ended its attribute early, and the name of a monomorphized node, which carries its type arguments in angle brackets, was written raw into the `name`, `top`, `concrete` and `abstract` attributes. Ampersands are no longer escaped twice over, which used to turn a `<` in a log message into `&amp;lt;`.
  - A stream of an uninterpreted sort reports its type as the schemas describe it: `abstrName` in XML, where the name used to be written as an attribute of its own with no value at all, and a quoted `typeInfo.name` in JSON, where it was unquoted.
  - A value of an uninterpreted sort, and any other value JSON has no literal of its own for, is written as a JSON string rather than bare.
