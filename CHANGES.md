...
- Support for [recursive functions](https://kind.cs.uiowa.edu/docs/main/user/inputs-and-outputs/lustre/#recursive-functions), declared with the `rec` modifier, including mutual recursion. Termination is established through a mandatory `decreases` clause over integer or ADT measures. A recursive function is unrolled a bounded number of times, up to `--rec_unrollings` (10 by default): outside of compositional analyses, the recursive calls past the unrollings are left unconstrained, and the function is unrolled further whenever a counterexample reaches one, without an analysis of its own; in a compositional analysis, they are abstracted by the function's contract, and when the analysis is also modular a call from another node is refined by unrolling the function one step at a time once the function's own analysis has proved its contract. A function with no contract to abstract it with, or declared `transparent`, is unrolled with its recursive calls left unconstrained in every analysis; an `opaque` one is always abstracted by its contract. A call applied to quantified variables is accepted for a function whose recursive calls are left unconstrained: the function is then defined at the SMT level with `define-funs-rec` (with Z3 or cvc5), so that the solver knows it at every argument.
